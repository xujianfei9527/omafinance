...
test("watchlist shows a dim 52-week drawdown after the change pill", () => {
  ...
  assert.match(list, /visibleColumnGap:\s*Style\.space\(30\)/)
  assert.match(list, /sparkEdgeCompensation:\s*Style\.space\(1\)/)
  assert.match(list, /rowColumnGap:\s*visibleColumnGap - sparkEdgeCompensation/)
  assert.match(list, /textTrailingCompensation:\s*Style\.space\(7\)/)
  assert.match(list, /nameColumnWidth:\s*Style\.space\(204\)/)
  assert.match(list, /priceColumnWidth:\s*Style\.space\(110\)/)
  assert.match(list, /id:\s*spark[\s\S]*?anchors\.left:\s*nameCol\.right[\s\S]*?anchors\.leftMargin:\s*listViewRoot\.rowColumnGap - listViewRoot\.textTrailingCompensation[\s\S]*?anchors\.right:\s*priceCol\.left[\s\S]*?anchors\.rightMargin:\s*listViewRoot\.rowColumnGap/)
  assert.match(list, /id:\s*priceCol[\s\S]*?width:\s*listViewRoot\.priceColumnWidth/)
